...
directional_paths = {}
for a in directional_keypad:
    for b in directional_keypad:
        directional_paths[(a, b)] = [
            path_to_moves(reconstruct_path(n))
            for n in a_star2(
                directional_keypad[a],
                directional_keypad[b],
                directional_keypad.values(),
            )
        ]

@cache
def get_length(seq, depth=25):
    if depth == 1:
        return sum(len(directional_paths[(a, b)][0]) for a,b in zip(A+seq, seq))
    length = 0
    for a,b in zip(A+seq, seq):
        length += min(get_length(next_seq, depth-1) for next_seq in directional_paths[(a,b)])
    return length
...
for line in lines:
    ...
    complexity += min(get_length(s,2) for s in seqs)*int(line[:-1])
print(complexity)
